...
// Where to find the NVIDIA runtimes. Empty strings mean "next to the exe".
struct RuntimePaths {
    std::wstring nrDir;   // folder containing nvngx_dlssnr.dll (Feature 18)
    std::wstring srDir;   // folder containing nvngx_dlss.dll (Feature 1, only for upscaling)
};
...
public:
    ...
    // Initialize D3D12 + NGX Feature 18 (DLSS-NR) [+ Feature 1 when upscaling]
    bool init(const VideoHeader& hdr, const RuntimePaths& paths = RuntimePaths{});

    // Live mode: apply per-frame settings. Re-creates Feature 18 when the model
    // preset changed. Returns false (with getLastError()) if the re-create failed.
    bool applyLiveSettings(const LiveSettings& s);

    // Process one frame with RGBA8 (legacy/live) or linear RGBA16F input and RG16F motion vectors.
    ...
    uint32_t getLastNgxResult() const { return m_lastNgx; }

    // Release NGX features, D3D12 objects and the loaded modules. Idempotent.
    void shutdown();

private:
    bool setupD3D12();
    bool allocateResources(uint32_t inW, uint32_t inH, uint32_t outW, uint32_t outH);
    bool initNGX(const VideoHeader& hdr, const RuntimePaths& paths);
    bool createNRFeature();
    void releaseNRFeature();
    bool executeAndWait();
    void waitGPU();
    ...
    // NGX Snippet & Shim modules + resolved entry points
    HMODULE m_coreMod  = nullptr;
    HMODULE m_nrMod    = nullptr;
    HMODULE m_shimMod  = nullptr;
    void*   m_nrCreate  = nullptr;
    void*   m_nrEval    = nullptr;
    void*   m_nrRelease = nullptr;
    void*   m_shimCreate  = nullptr;
    void*   m_shimEval    = nullptr;
    void*   m_shimRelease = nullptr;

    // Feature 18 (DLSS-NR)
    NGXParameter* m_params  = nullptr;
    NGXHandle*    m_feature = nullptr;
    uint32_t      m_createdPreset = 0;   // model preset Feature 18 was created with
    ...
    uint32_t      m_lastNgx = 0;
    bool          m_initialized = false;
    bool          m_rgba8 = false;   // RGBA8 in/out ('D5V4' legacy or 'D5L1' live)
    bool          m_live  = false;   // 'D5L1'

    // Optical Flow (DIS) State
    DISOpticalFlow       m_dis;
    std::vector<uint8_t> m_prev_gray;
    std::vector<uint8_t> m_flowScratch;
